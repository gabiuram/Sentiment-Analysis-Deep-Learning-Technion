...
class UCC_classifier(nn.Module):
  def __init__(self):
    ...
    base_model = AutoModelForCausalLM.from_pretrained(MODEL_PATH, return_dict=True, torch_dtype=torch.float16)
  ...
  def forward(self, input_ids, attention_mask):
    x = self.llama(input_ids=input_ids, attention_mask=attention_mask).logits
    x = torch.mean(x, 1)
    x = self.dropout(x)
    x = self.fc(x)
    return x
...
if __name__ == '__main__':
    ...
    Training.evaluate_saved_model(model_path, test_loader, test_data, "LLaMA2 + LoRA\nROC-AUC Score on Test Set", True, UCC_classifier)